...
import { Search, Plus, Truck, CheckCircle, X, Trash2, Barcode, Printer, ArrowLeft, LayoutGrid, Eye, Info, ChevronDown, Edit2, ArrowRight, UserCircle, PieChart } from 'lucide-react';
    ...
    <>
      ...
      <div className="flex-1 flex flex-col min-w-0 print:hidden">
        ...
        <div className="h-14 flex items-center px-4 bg-white border-b border-slate-200 shrink-0 gap-2">
          ...
          <h1 className="hidden sm:block text-lg font-bold text-slate-800 whitespace-nowrap">Nhập hàng</h1>
        ...
        <div className="flex-1 overflow-auto bg-white">
          ...
          <div className="md:hidden divide-y divide-slate-100">
            {cart.length === 0 ? (
              <div className="p-20 text-center text-slate-400 italic text-sm">Chưa có sản phẩm nào trong phiếu nhập.</div>
            ) : (
              cart.map((item, index) => (
                <div key={item.id} className="p-4 space-y-3">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-400 font-medium mt-0.5">Mã: {item.id}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Số lượng</label>
                      <input 
                        type="text" 
                        value={item.qty}
                        disabled={item.hasSerial}
                  ...
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-xs font-medium text-slate-500">Thành tiền:</span>
                    <span className="text-sm font-bold text-slate-800">{formatNumber((item.price * item.qty) - (item.discount || 0))}đ</span>
                  </div>
                    ...
                    <div className="space-y-2 pt-2 border-t border-slate-50">
                      <div className="relative">
                        ...
                        <input 
                          ...
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-3 py-1.5 text-xs outline-none focus:border-blue-400 font-medium"
                          onKeyPress={(e) => {
                            if (e.key === 'Enter') {
                              addSerialToItem(item.id, (e.target as HTMLInputElement).value);
                              (e.target as HTMLInputElement).value = '';
                      ...
                      {item.serials && item.serials.length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {(item.serials || []).map((sn, sIdx) => (
                            <span key={`${sn}-${sIdx}`} className="bg-blue-50 text-blue-600 text-[9px] font-bold px-2 py-0.5 rounded-md flex items-center gap-1 border border-blue-100">
      ...
      <div className="w-full lg:w-[380px] bg-white border-l border-slate-200 flex flex-col shrink-0 print:hidden lg:static fixed inset-y-0 right-0 transform lg:translate-x-0 translate-x-full transition-transform duration-300 ease-in-out z-[100]" id="import-summary">
        <div className="lg:hidden flex items-center justify-between p-4 border-b border-slate-100 bg-indigo-600 text-white">
          <h3 className="font-bold">Tổng kết nhập hàng</h3>
        ...
        <div className="p-4 flex justify-between items-center border-b border-slate-100">
          <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-100 transition-colors">
            <UserCircle size={20} className="text-slate-400" />
            <span className="text-sm font-semibold text-slate-700">Đặng Hoàng Tam</span>
        ...
        <div className="p-4 border-b border-slate-100">
          ...
          {selectedSupplier && (
            <div className="mt-3 p-3 bg-blue-50 rounded-xl flex justify-between items-center border border-blue-100 animate-in fade-in slide-in-from-top-1">
              <div>
